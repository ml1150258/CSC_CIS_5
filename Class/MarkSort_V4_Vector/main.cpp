/* 
 * File:   main.cpp
 * Author: mlehr
 * Created on Feb 1st, 2023, 10:10 AM
 * Purpose:  Custom Sort
 */

//System Libraries
#include <iostream>  //Input Output Library
#include <cstdlib>   //Random Function
#include <ctime>     //Seed the Random number function
#include <vector>    //Dynamic Memory
using namespace std;

//User Libraries

//Global Constants not Variables
//Science, Math, Conversions, Dimensions

//Function Prototypes
void filAray(vector<int> &);  //Fill an array with 2digit numbers
void prntAry(vector<int> &,int);//Print an array with #cols
void mrkSrt(vector<int> &);

//Execution begins here at main
int main(int argc, char** argv) {
    //Set random number seed
    srand(static_cast<unsigned int>(time(0)));
    
    //Declare Variables
    int size=50;
    vector<int> array(size);
    
    //Initialize Variables
    filAray(array);
    
    //Display Inputs/Outputs
    cout<<"The original unsorted array"<<endl;
    prntAry(array,10);
    
    //Map/Process the Inputs -> Outputs
    mrkSrt(array);
    
    //Display Inputs/Outputs
    cout<<endl<<endl<<"The sorted array"<<endl;
    prntAry(array,10);
    
    //Clean up memory and files
    
    //Exit the Program
    return 0;
}

void mrkSrt(vector<int> &a){
    for(int i=0;i<a.size()-1;i++){
        for(int j=i+1;j<a.size();j++){
            if(a[i]>a[j]){
                int temp=a[j];
                a[j]=a[i];
                a[i]=temp;
            }
        }
    }
}

void prntAry(vector<int> &a,int perLine){
    for(int i=0;i<a.size();i++){
        cout<<a[i]<<" ";
        if(i%perLine==(perLine-1))cout<<endl;
    }   
}

void filAray(vector<int> &a){
    for(int i=0;i<a.size();i++){
        a[i]=rand()%90+10;//[10,99]
    }
}